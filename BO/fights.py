...
class Fight:
    ...
    def __init__(self, playerTeam, enemyTeam = None):
        ...
        Menu.display("[p]{pokemon}[] de niveau {level} apparait".format(pokemon=enemyPokemon.getDisplayName(), level=enemyPokemon.getLevel()))
        ...
        while not self.done:
            ...
            if not self.done:
                ...
                if self.playerTeam[self.playerCursor].getHp() <= 0:
                    Menu.display("\n[y]{name} est KO.[]".format(name=self.playerTeam[self.playerCursor].getDisplayName()))
                    allTeamDown = True
                ...
                if self.enemyTeam[self.enemyCursor].getHp() <= 0:
                    Menu.display("\n[y]{name} est KO.[]".format(name=self.enemyTeam[self.enemyCursor].getDisplayName()))
                    allTeamDown = True
    ...
    def printField(self):
        ...
        Menu.display("\n[y]{name}[] Lv{level} [{hpcolor}]{hp}[]/{maxHp}        [b]vs[]        [{enemyhpcolor}]{enemyhp}[]/{enemymaxHp} [y]{enemyname}[] Lv{enemylevel} \n".format(
            hpcolor="r" if playerPokemon.getHp() <= playerPokemon.getMaxHp() / 4 else "y" if playerPokemon.getHp() <= playerPokemon.getMaxHp() / 2 else "g",
            enemyhpcolor="r" if enemyPokemon.getHp() <= enemyPokemon.getMaxHp() / 4 else "y" if enemyPokemon.getHp() <= enemyPokemon.getMaxHp() / 2 else "g",
    ...
    def makePlayerChooseFightOption(self):
        self.printField()
        playerChoice = Menu.ShowQuestion("Que va faire %s ?" % self.playerTeam[self.playerCursor].getDisplayName(), ["Attaquer", "Sac", "Pokemon", "Fuite"])
    ...
    def makePlayerChooseAttack(self):
        self.printField()
        self.playerAttack = Menu.ShowQuestion(
    ...
    def makePlayerChoosePokemon(self):
        self.printField()
        playerChoice = Menu.ShowQuestion(
    ...
    def makeComputerChooseAttack(self):
        self.enemyAttack = random.choice(range(len(self.enemyTeam[self.enemyCursor].getMoves()))) # random attack
    ...
    def applyDamage(self, sender, target, skill):
        ...
        stab = 1.5 if type1 == skill.getType().getName() or type2 == skill.getType().getName() else 1
        efficiency = skill.getType().ratioAgainst(target.getType1()) * (1 if type2 == None else skill.getType().ratioAgainst(target.getType2()))
        speedMod = sender.getSpeed() % 2
        critRate = ((sender.getSpeed() + (-speedMod if speedMod < 1 else 2-speedMod)) / 256)
        randomCrit = (random.randint(0, 100) / 100)
        crit = 1 if randomCrit > critRate else (2 * sender.getLevel() + 5) / (sender.getLevel() + 5)
        ...
        Menu.display("\n{name} utilise {move}".format(name=sender.getDisplayName(), move=skill.getDisplayName()))
        if crit > 1 and skillDamage != 0:
            Menu.display("[y]coup critique ![]")
        if efficiency == 0:
            Menu.display("mais cela n'a aucun effet")
        if efficiency < 1 and efficiency > 0 and skillDamage != 0:
            Menu.display("mais ce n'est pas très efficace")
        if efficiency >= 2 and skillDamage != 0:
            Menu.display("c'est très efficace !")
